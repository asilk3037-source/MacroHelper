...
public partial class AjudaViewModel : ObservableObject
{
    ...
    [
        ...
        new("Como configuro um webhook?", "Em Integrações, cadastre a URL e escolha o evento (macro usada, usuário criado, backup concluído). Use \"Testar\" para validar."),
        new("Onde vejo notificações do sistema?", "Em Notificações, no menu Sistema — backups, novos usuários e outros eventos aparecem ali e na sininho da barra superior."),
        new("Como faço backup dos meus dados?", "Em Configurações → Backup, clique em \"Fazer backup agora\". Pode ativar backup automático também."),
    ]);

    public string VersaoApp => "1.0.0";
}
